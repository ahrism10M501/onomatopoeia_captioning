...
# 테스트를 위한 데이터 준비
root_path = ''
sound_data = glob.glob(f"{root_path}\\test\\**\\*.wav", recursive=True)
sound_annot_data = glob.glob(f"{root_path}\\test\\**\\*.json", recursive=True)
...
# 모델 준비
model = yamnet.yamnet()
model.classifier = nn.Linear(in_features=1024, out_features=len(dataset.label_to_idx), bias=True)
model.to(device)
